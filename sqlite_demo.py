...
def insert(conn, session_id, user_id, ct):
    sql_insert = '''
    INSERT INTO
        `session`(`session_id`,`user_id`,`ct`)
    VALUES
        (?, ?, ?);
    '''
    # 参数拼接要用 ?，execute 中的参数传递必须是一个 tuple 类型
    conn.execute(sql_insert, (session_id, user_id, ct))
    print('插入数据成功')
...
def find_user(conn, session_id):
    ...
    WHERE
        session_id=?
    '''
    user_id = conn.execute(sql, (session_id,))


def main():
    # 指定数据库名字并打开
    db_path = 'session.sqlite'
